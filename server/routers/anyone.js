...
router.post('/submit', asyncHandler(async (req, res) => {
  ...
  if (
    ...
    req.body.gameModes.some((gameMode) => typeof gameMode !== 'number' || gameMode < 0 || gameMode > 3)
  ) {
  ...
  if (missingGameModes.size > 0) {
    return res.status(422).json({ error: `Beatmapset has no beatmaps in game mode ${[...missingGameModes].join(', ')}` });
  }
