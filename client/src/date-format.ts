...
function fullIsoString(dateOrIsoString: Date | string) {
  ...
  return dateOrIsoString.replace(/(?<=^\d{4}-\d{2}-\d{2}) /, 'T')
...
export function inputDateTime(dateOrIsoString: Date | string | null | undefined): string | undefined;
...
export function mySqlDateTime(dateOrIsoString: Date | string | null | undefined): string | undefined;
